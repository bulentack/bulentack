<p align="center">
  <img src="https://raw.githubusercontent.com/bulentack/bulentack/main/banner.png" alt="Bülent Açıkgöz - Cloud & DevOps Solution Architect" width="100%">
</p>

# 👋 Hi, I'm Bülent Açıkgöz
## 🧑‍💻 About Me

I’m a **Cloud & DevOps Solution Architect** with a strong background in **Kafka, Kubernetes, Terraform**, and **multi-cloud platforms (OCI, GCP, AWS)**.  
I design and automate **scalable, fault-tolerant data and application platforms** for telecom and finance environments — combining infrastructure as code, CI/CD pipelines, observability, and high availability.

💡 I’m particularly passionate about:
- Building **Kafka architectures** (KRaft, Confluent, multi-region, cluster linking)
- Automating infrastructure with **Terraform and Ansible**
- Deploying **Kubernetes & OpenShift** clusters with GitOps (ArgoCD, Tekton)
- Designing **monitoring and resilience** stacks (Prometheus, Grafana, Loki)

I’ve contributed to projects involving:
- **Oracle/Telia** — OCI + Kafka + OKE microservices for telecom billing  
- **Amadeus** — Cloud Data Fabric migration using OpenShift and Kafka  
- **Vodafone & Deutsche Bank** — Multi-cloud DevOps and CI/CD platforms  

📫 Reach me on [LinkedIn](https://www.linkedin.com/in/bulentacikgoz) or via email: **bcacikgoz@gmail.com**

---

## 🧰 Featured Projects
| Project | Description | Stack |
|----------|--------------|--------|
| [terraform-k3s-nginx](https://github.com/bulentack/terraform-k3s-nginx) | Automates NGINX deployment on k3s cluster with PV, PVC, ConfigMap, and Ingress | Terraform, Kubernetes |
| [kafka-kraft-monitoring](https://github.com/bulentack/kafka-kraft-monitoring) | Kafka KRaft cluster with JMX Exporter, Prometheus, and Grafana dashboards | Kafka, Helm, Prometheus |
| [oke-gitops-argocd](https://github.com/bulentack/oke-gitops-argocd) | GitOps pipeline demo for Oracle Kubernetes Engine using ArgoCD | OCI, ArgoCD, Helm |

---

## 📊 Tech Stack
`Terraform` • `Ansible` • `Kubernetes` • `Helm` • `Prometheus` • `Grafana` • `Kafka` • `GitLab CI/CD` • `OCI` • `GCP` • `AWS`

## 🏷️ Certifications & Tools

![Terraform](https://img.shields.io/badge/Terraform-7B42BC?logo=terraform&logoColor=white)
![Ansible](https://img.shields.io/badge/Ansible-EE0000?logo=ansible&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?logo=kubernetes&logoColor=white)
![OpenShift](https://img.shields.io/badge/OpenShift-EE0000?logo=redhatopenshift&logoColor=white)
![Kafka](https://img.shields.io/badge/Kafka-231F20?logo=apachekafka&logoColor=white)
![Prometheus](https://img.shields.io/badge/Prometheus-E6522C?logo=prometheus&logoColor=white)
![Grafana](https://img.shields.io/badge/Grafana-F46800?logo=grafana&logoColor=white)
![GitLab CI/CD](https://img.shields.io/badge/GitLab_CI%2FCD-FC6D26?logo=gitlab&logoColor=white)
![ArgoCD](https://img.shields.io/badge/ArgoCD-FE6D4C?logo=argo&logoColor=white)
![Helm](https://img.shields.io/badge/Helm-0F1689?logo=helm&logoColor=white)
![Oracle Cloud](https://img.shields.io/badge/Oracle_Cloud-F80000?logo=oracle&logoColor=white)
![GCP](https://img.shields.io/badge/Google_Cloud-4285F4?logo=googlecloud&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-FF9900?logo=amazonaws&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?logo=linux&logoColor=black)
![Bash](https://img.shields.io/badge/Bash-4EAA25?logo=gnubash&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)

---

## 🚀 My Focus
- Building resilient **Kafka clusters** across multi-cloud  
- Automating infrastructure provisioning and CI/CD pipelines  
- Designing **GitOps-based Kubernetes** deployments  
- Integrating observability with **Prometheus and Grafana**

---

💬 *"Automate everything, observe everything."*  
— **Bülent Açıkgöz**
---
📈 *Profile views and stats coming soon via GitHub Actions...*
